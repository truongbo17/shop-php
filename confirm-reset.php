...
if(isset($_GET['email']) && isset($_GET['token'])){
    ...
    $checkRMail = executeResult($sql,true);
    if($checkRMail != null){
        //get token check
        $token = getGET('token'); 
        $token = fixAllInput($token);
        //so sánh với database
        if($checkRMail['token'] == $token){
...
<head>
    ...
    <!-- Basic Page Needs
      ================================================== -->
      <meta charset="utf-8">
      <title>RESET | USER TRUONGBO</title>
  ...
  <body id="body">
                ...
                <div class="col-md-6 col-md-offset-3">
                    <div class="block text-center">
                        <section class="form login">
                            <h2 class="text-center">ĐẶT LẠI MẬT KHẨU | TRUONGBO</h2>
                            <p><?=$checkRMail['email']?></p>
                            <form action="#" method="POST" enctype="multipart/form-data" class="text-left clearfix" autocomplete="off">
                                ...
                                <div class="form-group">
                                    <input type="email" name="email" value="<?=$checkRMail['email']?>" hidden>
                                </div>
                                <div class="form-group">
                                    <input type="password" name="password" class="form-control" placeholder="Nhập mật khẩu mới" required>
                                </div>
                                <div class="text-center button">
                                    <button type="submit" class="btn btn-main text-center">ĐỔI MẬT KHẨU</button></button>
                                </div>
                            </form>
                            <p class="mt-20">Nhớ mật khẩu rồi à ?<a href="signin.php"> Trở lại đăng nhập</a></p>
    ...
    <script>
        ...
        continueBtn.onclick = () => {
                ...
                if (xhr.readyState === XMLHttpRequest.DONE) {
                    if (xhr.status === 200) {
                        ...
                        if (data === "success") {
                            ...
                            errorText.textContent = "Mật khẩu của bạn đã được thay đổi thành công,hãy đăng nhập !";
            ...
            formData.append('action', 'confirmResetPass');
            xhr.send(formData);
        }
    </script>
</body>
